...
public class OpenAction extends RestrictedAction {
	 ...
	 */
	public OpenAction(EnvironmentFrame frame) {
		super("Open...", null);
	...
	//			MAIN_MENU_MASK));
		...
		this.fileChooser.setFileFilter(new javax.swing.filechooser.FileFilter(){  
			...
			return (f.getName().endsWith(".jff")) || f.isDirectory();  
			}  
		...
		for (int i = 0; i < codecs.length; i++) {
			try {
				...
				if (!openOrRead) {
	    			...
	    			ef.getEnvironment().getLogArea().append("\n" + sdf.format(new Date()) + " - Open FSM: " + file.getAbsolutePath());
	    			/** LOGGER - End */
				}
				return;
			} catch (ParseException e) {
