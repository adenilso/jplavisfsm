...
//			bar.add(menu);
	...
	private static JMenu getFileMenu(EnvironmentFrame frame) {
		...
		if (Universe.CHOOSER != null) {
			...
			addItem(menu, new NewFrameEditorAction(frame));
			addItem(menu, new OpenAction(frame));
...
//					(gui.environment.GrammarEnvironment) environment));
	...
	/*
	 * The Menu intem of Generation Methods
	 * @author arineiza
	 */
	private static JMenu getTestSection(EnvironmentFrame frame) {
		Environment environment = frame.getEnvironment();
		JMenu menu = new JMenu("Test Section");
		Serializable object = environment.getObject();
			
		if(CreateTestSectionAction.isApplicable(object))
			addItem(menu, new CreateTestSectionAction ((automata.Automaton) object,
					environment));
				
		if(OpenTestSection.isApplicable(object))
			addItem(menu, new OpenTestSection ((automata.Automaton) object,
					environment));
		
		
//		/* Another Option */
//		JMenu subMenu = new JMenu("Generation Methods");
//		if(MethodGenerationAction.isApplicable(object)){
//			subMenu.add(new MethodGenerationAction ((automata.Automaton) object,
//					environment));
//			menu.add(subMenu);
//		}
	...
	private static JMenu getConfigMenu(EnvironmentFrame frame) {
		Environment environment = frame.getEnvironment();
		JMenu menu = new JMenu("Configuration");
		Serializable object = environment.getObject();
	
		addItem(menu, new ConfigurationAction(frame));
...
//			menu.add(new StateColorSelector(automaton,environment,menu));
	...
	private static JMenu getTestMenu(EnvironmentFrame frame) {
		...
		/*
		 * New Method - MEFs Completeness
		 * @ author arineiza
		 */		
		if(CompletenessAction.isApplicable(object))
			addItem(menu, new CompletenessAction ((automata.Automaton) object,
					environment));
		
		/*
		 * New Method - MEFs Connectivity
		 * @ author arineiza
		 */
		if(ConnectivityAction.isApplicable(object))
			addItem(menu, new ConnectivityAction ((automata.Automaton) object,
					environment));
		 ...
		 * @ author arineiza
		 */
		if(MinimalAction.isApplicable(object))
			addItem(menu, new MinimalAction ((automata.Automaton) object,
					environment));
...
//	}
	 ...
	 */
	private static JMenu getAboutMenu(EnvironmentFrame frame) {
		Environment environment = frame.getEnvironment();
...
//		addItem(menu, new AboutAction());
		...
		addItem(menu, new PanelAcknowledgementAction(frame));
