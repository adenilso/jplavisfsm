...
public class InputTableModel extends GrowableTableModel {
	...
	public static InputTableModel getModel(Automaton automaton, boolean multipleFile, boolean isEquivalentePane) {        
		InputTableModel model = (InputTableModel) INPUTS_TO_MODELS
				.get(new Integer(inputsForMachine(automaton)));
		
		if(isEquivalentePane){
			model = new InputTableModel(1);
			model.toEquivalentPane = isEquivalentePane;
		}
		else{
			if (model != null && (model.isMultiple == multipleFile)) {
				model = new InputTableModel(model);
    ...
    public boolean toEquivalentPane = false;
