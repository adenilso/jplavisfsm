...
//		this.getContentPane().add(environment, BorderLayout.CENTER);
		...
		this.environment.setLogArea(logArea);
		
		logArea.setText(" *********** JPlavisFSM Logger *********** \n");
		
		JPanel panel = new JPanel();
		panel.add(logArea, BorderLayout.SOUTH);
//		panel.add(new JScrollPane(logArea), BorderLayout.SOUTH);
		JSplitPane split = SplitPaneFactory.createSplit(false,0.34, environment, new JScrollPane(logArea));
...
//		this.getContentPane().add(new JScrollPane(logArea), BorderLayout.SOUTH);
...
//                  Get the suggested file name.
		...
		this.environment.getLogArea().append("\n" + sdf.format(new Date()) + " - FSM file: "+ file.getAbsolutePath());
		/** LOGGER - End */	
	...
	private static final String DEFAULT_TITLE = "JPlavisFSM";
