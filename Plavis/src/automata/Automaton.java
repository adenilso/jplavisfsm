...
//				a.addTransition(ts[i].copy(from, to));
	...
	public Object cloneJPlavis() {
		...
		while (it.hasNext()) {
			...
			map.put(state, nstate);
			a.addState(nstate);
			/*DEBUG*/ System.out.println("* "+nstate.toString());

		}
		// Set special states.
		a.setInitialState((State) map.get(getInitialState()));
		/*DEBUG*/ System.out.println("i "+getInitialState().toString());
		...
		while (it.hasNext()) {
			...
			for (int i = 0; i < ts.length; i++) {
                ...
                toBeAdded.setToState(to);
				a.addTransition(toBeAdded);
				/*DEBUG*/ System.out.println("* "+toBeAdded.toString());
			}
		}
		for(int k = 0; k < this.getNotes().size(); k++){
			Note curNote = (Note)this.getNotes().get(k);		
...
//				dest.addTransition(ts[i].copy(from, to));
	...
	public void addTransition(Transition trans) {
        ...
        State sta = trans.getToState();
        //list = (List)transitionToStateMap.get(trans.getToState()) ;
        list = (List)transitionToStateMap.get(sta) ;
		list.add(trans);
	...
	public void removeTransition(Transition trans) {
		...
		State st = trans.getFromState();
		//List l = (List) transitionFromStateMap.get(trans.getFromState());
		boolean b = transitionFromStateMap.containsKey(st);
		
		List l = (List) transitionFromStateMap.get(st);
		l.remove(trans);
